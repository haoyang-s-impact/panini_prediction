...
python analysis/model_comparison_report.py  # Outputs PNGs to output/

# Run price skewness analysis
python analysis/analyze_price_skewness.py   # Outputs PNG to output/
...
**Stage 3: ML Training** (`models/train_price_regressor.py`, `_v2.py`, `_v3.py`, `_v4.py`)
...
**Stage 4: Serving** (`serve/`, `app.py`, `models/train_production_model.py`)
...
- `serve/inference.py` — `predict_from_image(image_bytes)`: single function that chains EasyOCR → feature extraction → XGBoost prediction. Reuses extraction functions from `panini_card_ocr_etl.py` directly. Auto-detects GPU for EasyOCR.
...
- `app.py` — Streamlit frontend: upload image → display predicted price, extracted features, Claude analysis, raw OCR debug. Auto-detects GPU for EasyOCR.
...
- `analysis/model_comparison_report.py` — VS Code interactive report: trains all 4 model versions + ablation variants, generates comparison plots
